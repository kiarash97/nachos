...
Thread::Thread(Thread* parent,char* threadName)
{
    name = threadName;
    stackTop = NULL;
    stack = NULL;
...
#ifdef USER_PROGRAM
    space = NULL;
    threadID=machine->GetNewPid();
    parentID=parent->threadID;
    CopyRegistersFromParent(parent);
#endif
}

#ifdef USER_PROGRAM
//copye the parent registers from parent to child (esp for fork)
void Thread::CopyRegistersFromParent(Thread* parent)
{

    for (int i=0; i<NumTotalRegs; i++)
    {
        this->userRegisters[i] = parent->userRegisters[i];
    }
}
#endif
//----------------------------------------------------------------------
